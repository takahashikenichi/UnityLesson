...
public class Debri : MonoBehaviour {
	...
	void Start () {
				// シーン中のEarthオブジェクトにアクセスいて、EarthオブジェクトおTransformコンポーネントにアクセス
	...
	void Update () {
				...
				if(targetWay) {
						transform.RotateAround(targetPos, targetRot, -angle * Time.deltaTime);
				} else {
						transform.RotateAround(targetPos, targetRot, angle * Time.deltaTime);
				}
				// 軸に沿って少し回転させる
				transform.Rotate(targetRot, Time.deltaTime*360, Space.Self);
	}
}
